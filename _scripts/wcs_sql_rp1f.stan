...
transformed parameters {
  ...
  //  d[i] = Phi_approx( mu_pr[3] + sigma[3] * d_pr[i] ) * 5;
    f[i] = Phi_approx( mu_pr[3] + sigma[3] * f_pr[i] )*5;
  }
}
...
model {
  ...
  mu_pr ~ normal(0, 1);
  //sigma ~ normal(0, 0.05);
    sigma[1] ~ normal(0, 0.05);
    sigma[2]  ~ normal(0, 0.05);
 ...
 // d_pr ~ normal(0, 1);
  ...
  for (i in 1:N) {
    ...
    for (t in 1:Tsubj[i]) {
      ...
      if (outcome[i,t] == 1) {
        att_signal[1,1] = pow((subj_att .* choice_match_att[i,t])[1,1],f[i]) ;
        att_signal[1,2] = pow((subj_att .* choice_match_att[i,t])[1,2],f[i]) ;
        att_signal[1,3] = pow((subj_att .* choice_match_att[i,t])[1,3],f[i]) ;
        sum_att_signal = sum(att_signal);
        att_signal /= sum_att_signal;
        tmpatt = (1.0 - r[i])*subj_att + r[i]*att_signal;
      } else {
        att_signal[1,1] = pow((subj_att .* (unit - choice_match_att[i,t]))[1,1],f[i]) ;
        att_signal[1,2] = pow((subj_att .* (unit - choice_match_att[i,t]))[1,2],f[i]) ;
        att_signal[1,3] = pow((subj_att .* (unit - choice_match_att[i,t]))[1,3],f[i]) ;
        sum_att_signal = sum(att_signal);
        att_signal /= sum_att_signal;
      ...
      tmpatt[1, 1] = pow(subj_att[1, 1],1);
      tmpatt[1, 2] = pow(subj_att[1, 2],1);
      tmpatt[1, 3] = pow(subj_att[1, 3],1);
